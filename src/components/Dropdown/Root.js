...
export function DropdownRoot() {
  ...
  // First interaction
  const [hasIteracted, setHasIteracted] = useState(false);
  const isFirstInteraction = isActive && !hasIteracted;

  if (isFirstInteraction) {
    setTimeout(() => {
      if (!hasIteracted) setHasIteracted(true);
    }, 15);
  }
